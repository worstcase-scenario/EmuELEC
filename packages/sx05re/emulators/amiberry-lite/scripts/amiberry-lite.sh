...
AMIBERRY_CONFIG_FILE=${AMIBERRY_CONFIG_DIR}/amiberry-lite.conf
...
if [ `echo $1 | grep -i .zip | wc -l` -eq 1 ]; then
  ...
  if [ -f "$AMIBERRY_TMP_DIR"/*.*nfo ] && [ -f "$AMIBERRY_TMP_DIR"/*/*.*lave ]; then
    ...
    amiberry-lite -f -o amiberry_config=${AMIBERRY_CONFIG_FILE} "$AMIBERRY_TMP_CONFIG" --autoload "$1" >> $AMIBERRY_LOG 2>&1
    ...
    amiberry-lite -f -o amiberry_config=${AMIBERRY_CONFIG_FILE} "$AMIBERRY_TMP_CONFIG" >> $AMIBERRY_LOG 2>&1
  fi
...
# Check for .uae config file
  ...
  amiberry-lite -f -o amiberry_config=${AMIBERRY_CONFIG_FILE} "$1" >> $AMIBERRY_LOG 2>&1
