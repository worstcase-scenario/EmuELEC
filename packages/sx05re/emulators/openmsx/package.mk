...
# '    SDL_GetWindowSize(w,&ww,&wh);\n'
# '    if(ww<=0||wh<=0) return;\n'
# '    // Calculate 4:3 render area (works with and without full_stretch)\n'
# '    float rw=(float)wh*4.0f/3.0f;\n'
# '    float rh=(float)wh;\n'
# '    if(rw>(float)ww){rw=(float)ww;rh=rw*3.0f/4.0f;}\n'
# '    float ox=((float)ww-rw)*0.5f;\n'
# '    float oy=((float)wh-rh)*0.5f;\n'
# '    float cx=((float)mx-ox)/rw*2.0f-1.0f;\n'
# '    float cy=1.0f-((float)my-oy)/rh*2.0f;\n'
# '    // Hide cursor outside render area\n'
# '    if(cx<-1.0f||cx>1.0f||cy<-1.0f||cy>1.0f) return;\n'
# '    float sx=0.025f, sy=sx*(float)ww/(float)wh;\n'
# '    float v[]={cx,cy, cx+sx,cy-sy*0.5f, cx+sx*0.45f,cy-sy*1.3f};\n'
# '    GLint prevVbo;\n'
# '    glGetIntegerv(GL_ARRAY_BUFFER_BINDING,&prevVbo);\n'
