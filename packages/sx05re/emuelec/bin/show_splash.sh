...
if [[ -f "/storage/.config/emuelec/configs/novideo" ]] && [[ ${VIDEO} != "1" ]]; then
    if [ "${ACTION_TYPE}" != "intro" ]; then
        if [ "${SS_DEVICE}" == 1 ]; then
		    ${PLAYER} -fs -autoexit -loglevel error -nostats -vf "${FILTER_FILL}" "${SPLASH}" > /dev/null 2>&1
        else
            ${PLAYER} -fs -autoexit -loglevel error -nostats -vf "${FILTER_FILL}" -loop 1 "${SPLASH}" > /dev/null 2>&1
        fi
    fi 
else
    # Display intro video
